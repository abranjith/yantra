...
The production system prompt lives in `packages/agent/src/runtime/prompt.ts`. It has exactly six governed sections: role, scope, operating loop, trust boundary, safety, and completion/failure. Tool names, schemas, and provider mechanics are intentionally absent because the registered tool catalog is authoritative.

The **scope** section holds a run to what the goal asked for. It forbids unrequested actions and side experiments run to explain a surprising result, pins user-supplied values (never swapped for one read off a page, never retried with different inputs), makes an adverse outcome — a rejection, an error, an empty result — a finding to report rather than an obstacle to engineer around, and bounds "verify" to confirming the agent's own action landed. It narrows interpretation latitude over a broad goal to _how_ the goal is accomplished, so the anti-stall rule in completion/failure is preserved.

Every semantic prompt edit must also bump `PROMPT_VERSION` (currently `agent-v8`). Runs record both that version and a SHA-256 hash of the exact prompt text, so audits can distinguish prompt revisions. The per-run user prompt contains only the sanitized goal, an ambient context block, enforced constraints, allowed hosts/scope, and bounded approved profile context; page content and tool results remain untrusted data.
