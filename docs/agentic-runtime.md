...
Every semantic prompt edit must also bump `PROMPT_VERSION` (currently `agent-v7`). Runs record both that version and a SHA-256 hash of the exact prompt text, so audits can distinguish prompt revisions. The per-run user prompt contains only the sanitized goal, an ambient context block, enforced constraints, allowed hosts/scope, and bounded approved profile context; page content and tool results remain untrusted data.
